	...
	public void RefundS() { // 환불 진행하는 함수
		...
		while (true) {
			do { // 문법 형식에 맞는 문자열을 입력받을구입 날짜 때 까지 입력받는 do-while문
				System.out.print("결제 코드를 입력하세요(종료하고싶다면 완료를 입력하시오): ");
				PayCode = scan.nextLine();
			} while (!(checkL_PayCode(PayCode) & checkI_PayCode(PayCode) & checkBlank(PayCode)
					& checkR_PayCode(PayCode)));
			if (PayCode.equals("완료")) {
				System.out.println("환불을 종료하고 메인메뉴로 이동합니다.");
				return;
			}
			...
			while (true) {
				...
				if ((checkL_Product_code(product_code))
						& (checkC_Product_code(product_code) & checkBlank(product_code))) { // //
																							// 검사
					for (Product i : list) {
				...
				if (Product_list.containsKey(PayCode)) { // 결제 코드가 있으면 참 없으면 거짓
					isOtherPC=Product_list.containsKey(PayCode);// 여기서 저 함수 출력
					continue;
				} else
					break;
			}
